...
use poker::{winning_hands, category_of_hand_str, Rank};
use poker::HandCategory::*;
...
#[test]
fn test_category_of_straight_flush() {
    assert_eq!(category_of_hand_str("4S 5S 6S 7S 8S"), StraightFlush(Rank::from("8")));
}

#[test]
fn test_category_of_four_of_a_kind() {
    assert_eq!(category_of_hand_str("4C 4D 4H 4S 3C"), FourOfAKind(Rank::from("4"), Rank::from("3")));
}

#[test]
fn test_category_of_full_house() {
    assert_eq!(category_of_hand_str("4C 4D 4H 5S 5C"), FullHouse(Rank::from("4"), Rank::from("5")));
}

#[test]
fn test_category_of_flush() {
    assert_eq!(category_of_hand_str("2C 4C 5C 6C 7C"), Flush(
        [Rank::from("7"), Rank::from("6"), Rank::from("5"), Rank::from("4"), Rank::from("2")]));
}

#[test]
fn test_category_of_straight() {
    assert_eq!(category_of_hand_str("4S 5S 6S 7S 8D"), Straight(Rank::from("8")));
}

#[test]
fn test_category_of_straight_with_low_ace() {
    assert_eq!(category_of_hand_str("AS 2S 3S 4S 5D"), Straight(Rank::from("5")));
    assert_eq!(category_of_hand_str("AS 2S 4S 5S 6D"), HighCard([Rank::from("A"),
        Rank::from("6"),
        Rank::from("5"),
        Rank::from("4"),
        Rank::from("2"),
...
#[test]
fn test_category_of_three_of_a_kind() {
    assert_eq!(category_of_hand_str("4S 4D 4C 3S 2D"), ThreeOfAKind(
        [Rank::from("4"), Rank::from("3"), Rank::from("2")]
    ));
}

#[test]
fn test_category_of_two_pair() {
    assert_eq!(category_of_hand_str("4S 4D 5C 5S 2D"), TwoPair(
        [Rank::from("5"), Rank::from("4"), Rank::from("2")]
    ));
}

#[test]
fn test_category_of_one_pair() {
    assert_eq!(category_of_hand_str("4S 4D 5C 10S 2D"), OnePair(
        [Rank::from("4"), Rank::from("10"), Rank::from("5"), Rank::from("2")]
    ));
}


#[test]
fn test_category_of_high_card() {
    assert_eq!(category_of_hand_str("4C 5D 6H 7S 9S"), HighCard([
        Rank::from("9"),
        Rank::from("7"),
        Rank::from("6"),
        Rank::from("5"),
...
#[test]
fn test_compare_within_categories() {
    assert_eq!(StraightFlush(Rank::from("9")) > StraightFlush(Rank::from("8")), true);

    let bigger = FullHouse(Rank::from("5"), Rank::from("4"));
    let smaller = FullHouse(Rank::from("5"), Rank::from("3"));
    assert_eq!(bigger > smaller, true);
}


#[test]
fn test_compare_different_categories() {
    assert_eq!(StraightFlush(Rank::from("8")) > Straight(Rank::from("8")), true);
}
